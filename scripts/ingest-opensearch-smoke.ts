...
// Force refresh so the document is immediately searchable
await fetch(`${ENDPOINT}/${INDEX}/_refresh`, { method: "POST" });
...
logger.info(
  { count, hits: result.hits.hits },
  `✅ Smoke test passed — found ${count} document(s) in OpenSearch`,
);
