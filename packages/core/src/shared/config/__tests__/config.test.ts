...
function clearEnv() {
  ...
  delete process.env.TRIAGE_ENABLED;
...
// SSM unavailable: silence console.error (expected behavior in this test)
  ...
  it('silently continues if SSM fetch fails (catches error)', async () => {
    ...
    it('paginates through NextToken until exhausted', async () => {
      mockSend
        ...
        .mockResolvedValueOnce({
          Parameters: [{ Name: '/junando/llm-api-key', Value: 'sk-page-2' }],
        });
      ...
      expect(config.llmProvider).toBe('claude');
      expect(config.llmApiKey).toBe('sk-page-2');
    });
  });
  ...
  describe('triage config', () => {
    it('defaults triageEnabled to false when env var is absent', async () => {
      setEnv({ ...validConfig });
      const config = await loadConfig();
      expect(config.triageEnabled).toBe(false);
      expect(config.triageProvider).toBeUndefined();
      expect(config.triageModel).toBeUndefined();
      expect(config.triageApiKey).toBeUndefined();
    });

    it('parses TRIAGE_ENABLED=true', async () => {
      setEnv({ ...validConfig });
      ...
      const config = await loadConfig();
      expect(config.triageEnabled).toBe(true);
    });

    it('parses TRIAGE_ENABLED=1 as true', async () => {
      setEnv({ ...validConfig });
      process.env['TRIAGE_ENABLED'] = '1';
      process.env['TRIAGE_PROVIDER'] = 'vercel-gateway';
      process.env['TRIAGE_MODEL'] = 'anthropic/claude-3-5-haiku';
      process.env['TRIAGE_API_KEY'] = 'triage-key';
      const config = await loadConfig();
    ...
    it('requires TRIAGE_PROVIDER, TRIAGE_MODEL, and TRIAGE_API_KEY when triageEnabled is true', async () => {
      setEnv({ ...validConfig });
      process.env['TRIAGE_ENABLED'] = 'true';
      await expect(loadConfig()).rejects.toThrow(/TRIAGE_PROVIDER is required/);
      await expect(loadConfig()).rejects.toThrow(/TRIAGE_MODEL is required/);
      await expect(loadConfig()).rejects.toThrow(/TRIAGE_API_KEY is required/);
    });

    it('does not require triage fields when triageEnabled is false', async () => {
      setEnv({ ...validConfig });
      process.env['TRIAGE_ENABLED'] = 'false';
      const config = await loadConfig();
      expect(config.triageEnabled).toBe(false);
    });
  });
