...
import { AlertmanagerPayloadSchema, normalizePayload } from '@junando/core'
...
export const handler = async (
  ...
  if (event.rawPath === '/health') {
    return { statusCode: 200, body: JSON.stringify({ status: 'ok', service: 'junando-webhook' }) }
  }
  ...
  if (!parsed.success) {
    return {
      statusCode: 422,
      body: JSON.stringify({ error: 'Invalid payload', issues: parsed.error.issues }),
    }
  }
